...
mod tests {
    ...
    fn inner_halo_proof_tests() {
        ...
        let t_0: ScalarField = Utils::get_n_random_scalars_not_zero(1, &mut rng)[0];
        let t_1: ScalarField = Utils::get_n_random_scalars_not_zero(1, &mut rng)[0];
        let t_2: ScalarField = Utils::get_n_random_scalars_not_zero(1, &mut rng)[0];
        ...
        let inner_halo_proof: InnerHaloProof =
            InnerHaloProver::new(&g_vec, &h, &t_comm, &rand_r, &t_hat, &t_vec, &b_vec, &u)
                .generate_proof(&mut prover_trans);

        let inner_halo_result: Result<(), Error> =
            InnerHaloVerifier::new(&g_vec, &b_vec, &h, &t_comm, &t_hat, &u)
