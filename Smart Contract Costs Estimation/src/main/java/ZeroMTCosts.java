...
public class ZeroMTCosts {
    ...
    public static void main(String[] args) throws IOException {
        ...
        lines.add(new String[]{"n", "m", "range_costs", "ipa_costs", "sigma_ab_costs", "sigma_r_costs", "sigma_sk_costs", "sigma_y_costs", "total_costs"});
        ...
        for (int n = 16; n <= 64; n *= 2) {
            for (int m = 2; m <= 64; m *= 2) {
                lines.add(new String[]{
                        String.format("%,d", n),
                        String.format("%,d", m),
                        String.format("%,d", range(opt, n, m)),
                        String.format("%,d", ipaProtOneCosts(opt) +
                                ipaProtTwoMultiexp(opt, n * m) +
                                rangeIpaArguments(opt, n, m)),
                        String.format("%,d", sigmaAB(opt, n, m)),
                        String.format("%,d", sigmaR(opt, n, m)),
                        String.format("%,d", sigmaSk(opt, n, m)),
                        String.format("%,d", sigmaY(opt, n, m)),
                        String.format("%,d", ipaProtOneCosts(opt) +
                                ipaProtTwoMultiexp(opt, n * m) +
                                range(opt, n, m) +
                                rangeIpaArguments(opt, n, m) +
        ...
        try (CSVWriter writer = new CSVWriter(new FileWriter("./costs.csv"))) {
